...
test('admin has no public registration and reports missing configuration safely',async({page})=>{await page.goto('/admin/login');await expect(page.getByRole('heading',{name:'Masuk ke dashboard'})).toBeVisible();await expect(page.getByText('Tidak ada registrasi publik')).toBeVisible();await expect(page.getByRole('button',{name:/Masuk/})).toBeDisabled()});
